...
COMMENT ON COLUMN t_qa_history.retrieved_chunks IS '检索到的相关文档块ID列表';
...
-- 插入演示用户
INSERT INTO t_user (id, username, email, is_active) 
VALUES 
    ('00000000-0000-0000-0000-000000000001', 'user_demo', 'demo@example.com', true),
    ('00000000-0000-0000-0000-000000000002', 'admin', 'admin@example.com', true)
...
-- GRANT CONNECT ON DATABASE rag_qa_system TO rag_readonly;
-- GRANT USAGE ON SCHEMA public TO rag_readonly;
-- GRANT SELECT ON ALL TABLES IN SCHEMA public TO rag_readonly;
-- GRANT SELECT ON ALL SEQUENCES IN SCHEMA public TO rag_readonly;
...
-- 检查用户数据
SELECT username, email, is_active, created_at FROM t_user;

-- 显示数据库大小
SELECT pg_size_pretty(pg_database_size('rag_qa_system')) as database_size;

-- ============================================
-- 初始化完成
-- ============================================
\echo '=========================================='
\echo 'RAG QA System 数据库初始化完成！'
\echo '=========================================='
